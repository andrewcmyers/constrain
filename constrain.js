...
// A part of a figure, including an entire figure or just a frame. Supports
// creators for building various parts of a figure.

// There is a set of Variables that can be adjusted to minimize an objective
// function generated by summing costs from various constraints.
//
// There is a set of GraphicalObjects that know how to render themselves based
// on associated Variables. There is also a set of constraint objects that are
// defined in terms of Variables and that can report a cost.
//
// Expressions can be built out of Variables. Constraints are mostly just Expressions
// that are supposed to be zero. The solver tries to get all constraint
// expressions as close to zero as possible.

const Figures = []

class Figure {
    // Create a figure appearing in a canvas.
    // 'canvas' should be either the canvas HTML element or its string id
    constructor(canvas) {
        this.figure = this
...
class Variable extends Expression {
    constructor(figure, basename, hint) {
        ...
        this.index = figure.numVariables
        this.figure = figure
...
// A Linear can interpolate between two numbers or between any two expressions
// that evaluate to 1-D arrays, such as Points.
...
function zeroCost(valuation, doGrad) {
    return doGrad ? [0, zeros(valuation.length)] : 0
}
...
// Create an ellipse path with center (x,y) and x radius rx, y radius ry
    ellipse: function(ctx, x, y, rx, ry) {
        ...
        ctx.beginPath()
        ctx.moveTo(x, y - ry)
        ctx.bezierCurveTo(x + rxk, y - ry,  x + rx, y - ryk,  x+rx, y)
        ctx.bezierCurveTo(x + rx, y + ryk,  x + rxk, y + ry, x, y + ry)
        ctx.bezierCurveTo(x - rxk, y + ry, x - rx, y + ryk, x - rx, y)
        ctx.bezierCurveTo(x - rx, y - ryk, x - rxk, y - ry, x, y - ry)
        ctx.closePath()
    }
...
function circlePath(ctx, x,y,r) {
    ...
    ctx.moveTo(x+r, y);
    ctx.bezierCurveTo(x+r, y+rk,  x+rk, y+r,  x, y+r);
    ctx.bezierCurveTo(x-rk, y+r,  x-r, y+rk,  x-r, y);
    ctx.bezierCurveTo(x-r, y-rk,  x-rk, y-r,  x, y-r);
    ctx.bezierCurveTo(x+rk, y-r,  x+r, y-rk,  x+r, y);
}
