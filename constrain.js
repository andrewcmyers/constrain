...
// A Figure is attached to a single canvas and knows how to render itself. It
// has a set of one or more associated Frames which can be stepped through.
...
// be changed later.
// 
// Associated with each Figure is a set of Variables that can be adjusted to
// minimize an objective function generated by summing costs from various
// constraints. Depending on which frame a figure is on, different variables
// are active and solved for.
//
// Expressions can be built out of Variables. Constraints are built using
// expressions.  They have a cost that is supposed to be as small as possible
// (zero, if the constraint is fully satisfiable). The solver tries to minimize
// the total cost of all constraints.
// 
class Figure {
